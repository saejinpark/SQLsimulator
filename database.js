...
class Database {
    ...
    create(data) {
        if (!/(TABLE|table)\s\w+\s\((\w|,|\s)+\)/.test) return false;
    ...
    insert(data) {
        if (!/(INTO|into)\s\w+\s\(.+?\)\s(VALUES|values)\s\(.+?\)/.test(data)) return false;
    ...
    delete(data) {
        if (!/(FROM|from)\s\w+\s(WHERE|where)\s\w+\s=\s[^\s=]+/.test(data)) return false;
        const [_from, name, _where, col, value] = data.match(/[^\s=]+/g);
        if (!this.warehouse.has(name)) return false;
        const mart = this.warehouse.get(name);
    ...
    update(data) {
        if (!/\w+\s(SET|set)\s\w+\s=\s\w+\s(WHERE|where)\s\w+\s=\s[^\s=]+/.test(data)) return false;
        const [name, _set, col, value, _where, findCol, findVal] = data.match(/[^\s=]+/g);
        if (!this.warehouse.has(name)) return false;
        const mart = this.warehouse.get(name);
    ...
    select(data) {
        if (/\*\s(FROM|from)\s\w+/.test(data)) {
            let [_from, name] = data.match(/\w+/g);
            if (!this.warehouse.has(name)) return false;
            const dummy = this.warehouse.get(name);
            dummy["name"] = name;
            return dummy;
        } else if (/\w+\s(FROM|from)\s\w+\s(WHERE|where)\s\w+\s[=<>]\s[^\s=]+/.test(data)) {
    ...
    drop(data) {
        if (!/(TABLE|table)\s\w+/.test(data)) return false;
